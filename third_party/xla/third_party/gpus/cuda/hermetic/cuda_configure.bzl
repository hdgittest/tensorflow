...
def _list_cuda_copts(compute_capabilities):
    copts = []
...
# Function works only in pair with non-hermetic toolchain
def _list_cuda_ptx_copts(repository_ctx, cuda_version):
    copts = []
...
def _create_dummy_cuda_repository(repository_ctx):
    ...
    repository_ctx.template(
        ...
        repository_ctx.attr.build_defs_tpl,
        {
            ...
            "%{cuda_version}": "0.0",
            "%{hermetic_cc_toolchain}": str(_use_hermetic_toolchains(repository_ctx)),
        },
    )
    ...
    repository_ctx.template(
        ...
        repository_ctx.attr.build_defs_tpl,
        {
            ...
            "%{cuda_extra_copts}": str(
                _list_cuda_ptx_copts(repository_ctx, cuda_config.cuda_version) +
                _list_cuda_copts(cuda_config.compute_capabilities),
            ),
...
def _create_toolchains_repository(repository_ctx):
    if enable_cuda(repository_ctx) or _is_linux_x86_64(repository_ctx):
        _create_local_toolchains_repository(repository_ctx)
    else:
